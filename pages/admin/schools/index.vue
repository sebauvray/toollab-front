  ...
  <div class="p-8">
    <div class="flex justify-between items-center mb-6">
      <div>
        ...
        <p class="text-gray-600 text-sm">{{ schools.length }} école{{ schools.length > 1 ? 's' : '' }} sur la plateforme</p>
      </div>
      <NuxtLink
    ...
    <div v-if="errorMsg" class="bg-red-50 border border-red-200 text-red-700 p-4 rounded mb-6">
      {{ errorMsg }}
    </div>
    ...
    <div v-else class="bg-white rounded-lg border">
      <table class="w-full">
        <thead class="bg-gray-50 border-b">
          <tr>
            <th class="text-left px-6 py-3 text-xs font-bold text-gray-600 uppercase">École</th>
            <th class="text-left px-6 py-3 text-xs font-bold text-gray-600 uppercase">Ville</th>
            <th class="text-left px-6 py-3 text-xs font-bold text-gray-600 uppercase">Email</th>
            <th class="text-left px-6 py-3 text-xs font-bold text-gray-600 uppercase">Actif</th>
          </tr>
        </thead>
        <tbody>
          ...
          <tr v-if="schools.length === 0">
            <td colspan="4" class="text-center py-12 text-gray-500">Aucune école.</td>
          </tr>
        </tbody>
      </table>
    </div>
