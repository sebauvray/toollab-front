...
<template>
  <PageContainer>
    <div v-if="isLoading" class="py-12 text-center text-gray-500 text-sm">Chargement…</div>
    <div v-else-if="error" class="bg-red-50 text-red-700 p-3 rounded">{{ error }}</div>

    <template v-else>
      <div class="flex items-center justify-between gap-4 mb-6">
        <div class="min-w-0">
          <h1 class="text-2xl font-bold text-gray-900 truncate">{{ classroom?.name }}</h1>
          <p class="text-sm text-gray-500">{{ students.length }} élève{{ students.length > 1 ? 's' : '' }}</p>
        </div>
        <button
        ...
        >
          <span
              ...
              class="inline-flex items-center justify-center min-w-[1.5rem] h-6 px-1.5 rounded-full bg-white/25 text-xs font-bold"
          >
            {{ decisionsCount }}
          </span>
          {{ isSaving
      ...
      <div v-if="yearClosed" class="mb-4 px-3 py-2 rounded-md bg-gray-100 text-gray-700 text-sm">
        Année clôturée — lecture seule
      </div>
      <div v-else-if="!outcomesOpen" class="mb-4 px-3 py-2 rounded-md bg-amber-50 text-amber-800 text-sm border border-amber-200">
      ...
      <div class="bg-white rounded-lg border border-gray-200 overflow-hidden">
        <table class="min-w-full">
          ...
          <tr>
            ...
            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase w-2/5">Décision</th>
