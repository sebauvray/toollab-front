...
const handleSubmit = async () => {
  ...
  try {
    ...
    const response = await fetch(`${useRuntimeConfig().public.apiUrl}/api/set-password`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Accept': 'application/json'
    ...
    message.value = { type: 'error', text: 'Une erreur est survenue lors de la définition du mot de passe' }
  } finally {
  ...
  <div class="flex flex-col items-center justify-start w-full h-full min-h-screen font-nunito">
    ...
    <div class="flex flex-col items-center justify-start bg-white rounded-3xl border w-[85%] sm:w-[70%] lg:w-[50%] 2xl:w-[35%] min-h-[50vh] shadow-xl">
      <form
        ...
        <h2 class="text-default text-[32px] font-bold mb-4">Création de votre compte</h2>
